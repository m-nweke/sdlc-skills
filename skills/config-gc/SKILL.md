---
name: config-gc
description: Garbage collection for your Claude Code configuration. Periodically scans ~/.claude (and ~/.claude-personal, on machines with a split config) for redundant, stale, orphaned, or low-value skills, memory, hooks, permissions, MCP servers, and caches, then walks the user through a confirm-each-deletion cleanup. Use when the user says "clean up my config", "config GC", "too many skills", "audit my setup", "my .claude is bloated", or asks for a periodic config review.
---
...
- After installing a large skill pack (like a batch vendoring pass into this repo), to reconcile overlaps with what already existed
...
|---|---------|------|--------------------------------|
| 1 | Skills | `~/.claude/skills/*/`, `~/.claude-personal/skills/*/` if present | Heavily overlapping names; never triggered in recent transcripts; domain mismatch with the user's actual work; broken or empty SKILL.md; **a symlink whose target no longer exists** in a repo like `sdlc-skills` |
...
4. **Soft-delete confirmed items**: prefer `.disabled` rename for skills/hooks and `_gc_trash/<date>/` move for files. For a skill symlinked from `sdlc-skills`, disabling means removing the symlink from the config dir(s) and optionally moving the real target in the repo to a `_gc_trash/` equivalent there too — the symlink alone isn't the source of truth. Permission entries live in JSON (no comments possible): back up the settings file, record each removed entry verbatim in `gc_log.md`, then remove it from the `allow` array with `jq`. Only hard-delete when the user explicitly asks.
...
Broken skill symlinks (channel 1) — a symlink whose repo target has been removed or renamed:

```bash
for f in ~/.claude/skills/* ~/.claude-personal/skills/*; do
  [ -L "$f" ] && [ ! -e "$f" ] && echo "BROKEN SYMLINK: $f -> $(readlink "$f")"
done
```
...
- **Touching anything outside `~/.claude`, `~/.claude-personal`, or the project's `.claude/`.** Config GC never wanders into source trees — and never touches `sdlc-skills/` itself beyond what a confirmed skill deletion requires.

## Best Practices

- Run after big additions, not just on a calendar: vendoring several skills into `sdlc-skills` at once is exactly when overlap with existing skills appears.
...
- `context-budget` — the additive/measurement half of the same lifecycle: finds and ranks the bloat that this skill then removes with confirmation.
