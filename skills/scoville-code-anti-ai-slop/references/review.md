...
Load this for a requested branch/PR/diff review, or any "review since X" ask. This is
the **Advise** mode mechanism for the review outcome: report only, edit nothing unless
...
this skill aggregates their findings afterward. Never merge or rerank findings across
axes — a change can cleanly pass one and fail the other, and collapsing them into one
verdict hides that.
...
comparison is against the merge-base). Also note the commit list via
`git log <fixed-point>..HEAD --oneline`.

Confirm the fixed point resolves (`git rev-parse <fixed-point>`) and the diff is
non-empty before spawning sub-agents — a bad ref or empty diff should fail here, not
...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.),
   fetched via whatever issue-tracker workflow the repo documents.
2. A path the user passed as an argument.
3. A spec file under `docs/`, `specs/`, or `.scratch/` matching the branch name or
   feature.
4. If nothing is found, ask the user where the spec is. If there isn't one, the Spec
   sub-agent skips and reports "no spec available."

### 3. Identify the standards sources

Anything in the repo documenting how code should be written (`CODING_STANDARDS.md`,
`CONTRIBUTING.md`, etc.).
...
- The standards-source files found in step 3, plus the smell baseline pasted in full
  (the sub-agent has no other access to it).
- The brief: "Report, per file/hunk where relevant, (a) every place the diff violates
  a documented standard: cite the standard (file + rule); and (b) any baseline smell
  you spot: name it and quote the hunk. Distinguish hard violations from judgement
  calls: documented-standard breaches can be hard, but baseline smells are always
  judgement calls, and a documented repo standard overrides the baseline. Skip
...
each axis* (if any). Don't pick a single winner across axes — that's the reranking
the separation exists to prevent.

Fold the result into this skill's evidence rules: lead with the observable result,
distinguish observation from inference, and never claim a check ran that didn't.
